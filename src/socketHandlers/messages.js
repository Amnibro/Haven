...
module.exports = function register(socket, ctx) {
  ...
  const FORUM_SELECT = `
    SELECT m.id, m.content, m.created_at, m.reply_to, m.edited_at, m.is_webhook, m.webhook_username, m.webhook_avatar, m.imported_from, m.is_archived, m.poll_data, m.burn_seconds, m.burning_started_at, m.persona_id, m.persona_username, m.persona_avatar, m.break_chain, m.ferry_target, m.type, m.title, m.tags,
  ...
  socket.on('send-message', (data) => {
    ...
    const channel = db.prepare('SELECT id, name, slow_mode_interval, text_enabled, voice_enabled, media_enabled, read_only, is_dm, is_forum, forum_tags FROM channels WHERE code = ?').get(code);
  ...
  socket.on('set-topic-meta', (data) => {
    ...
    try {
      ...
      io.to(`channel:${msg.code}`).emit('topic-updated', { channelCode: msg.code, messageId: msg.id, title: title || null, tags });
    } catch (err) {
